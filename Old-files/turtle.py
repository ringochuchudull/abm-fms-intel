...
def distance(t):
    ...
    return np.sqrt(t[0]*t[0] + t[1]*t[1])

#Iniitalise initial variables
for i in range(N_turtle):
    turtles.append((0,0)); 
...
for t in range(iteration):
    ...
    avg_distance = 0 
    for i in range(N_turtle):
        cur_turtle = turtles[i]

        direction = random.uniform(0, 2*math.pi)
        dx, dy = STEP_SIZE*math.sin(direction), STEP_SIZE*math.cos(direction)

        old_x, old_y = cur_turtle[0], cur_turtle[1]
        new_x, new_y = old_x +dx, old_y +dy
        new_turtle = (new_x, new_y)

        turtles[i] = new_turtle
        avg_distance += distance(new_turtle)

    avg_distance = avg_distance/N_turtle 
    distance_arr.append(avg_distance)
    
plot_x = [x[0] for x in turtles ]
plot_y = [y[1] for y in turtles ]
